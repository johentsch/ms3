...
class TestParseObject():
    ...
    def test_parse_keys(self, parse_obj, expected_keys):
...
class TestInstantiation():
    ...
    def test(self, expected_keys):
        assert isinstance(parsed_parse_obj, ms3.Parse)
...
def test_parsed(parsed_parse_obj):


def test_concatenated_facets(parsed_parse_obj):
    notes = parsed_parse_obj.notes()
    assert len(notes) > 0
